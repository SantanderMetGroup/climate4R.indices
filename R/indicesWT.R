...
#' @param cluster.type Clustering type that matches with the WT circulation index to be computed. 
#' @param centers Integer value indicating the number of clusters, \strong{k}, or center points. See \link[TransformeR]{clusterGrid} for more details.
...
indicesWT <- function(grid, season, cluster.type, centers=centers 
                      #rot=rot, members=members
                      ){
  
  cluster.type <- match.arg(cluster.type, choices = c("kmeans", "som", "hierarchical","lamb"), several.ok = FALSE)
